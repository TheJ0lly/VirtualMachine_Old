...
Instruction *Fibonacci_Test(VM *vm, int iter) {
    ...
    Fibonacci[2] = (Instruction)MOVE_INST(1, vm->registers.RC);
    ...
    Fibonacci[8] = (Instruction)CMP_VAL(iter, vm->registers.RC);   // Change value here to make it loop a certain number of times. 0 <= - will be infinite.
    Fibonacci[9] = (Instruction)APPEND_VAL_INST(1, vm->registers.RC);
